...
├── CHANGELOG.md         # Generated by auto-changelog during `npm run release`
├── package.json
├── README.md
└── tsconfig.json
```
...
```bash
npm run dev          # sanity-check the node in a local n8n instance
npx n8n-node lint    # lint (same check CI runs)
npm login            # authenticate npm (release-it checks npm auth once --publish is used)
npm run build        # compile TypeScript and copy icons to dist/
npm run release      # interactive n8n-node release (release-it)
```

`npm run release` (`n8n-node release`) wraps `release-it`. Run **plainly** (no `--publish` flag) it never touches npm — it only bumps the version, updates `CHANGELOG.md`, commits, tags, and pushes. Passing `--publish` (i.e. `npm run release -- --publish`) additionally runs npm's auth/collaborator checks — hence `npm login` above — and adds an interactive "publish to npm" step.

`release-it` walks through one prompt per step and expects `y`/`n` (default is **Yes** for all of them). A common answer sequence — bump, commit, tag and push everything, but let the **GitHub Actions** workflow do the actual npm publish with provenance instead of your own machine — is:

| # | Prompt | Typical answer | Why |
|---|--------|-----------------|-----|
| 1 | `Select increment (next version):` | *(pick patch/minor/major)* | Not yes/no — arrow keys + Enter |
| 2 | `Publish <package>@latest to npm?` | **No** | Skip publishing from your machine (no provenance) — CI publishes instead |
| 3 | `Commit (Release x.y.z)?` | **Yes** | Commit the version bump + changelog |
| 4 | `Tag (vx.y.z)?` | **Yes** | Create the `vx.y.z` git tag |
| 5 | `Push?` | **Yes** | Push the commit and tag to `origin` |
| 6 | `Create a release on GitHub (Release x.y.z)?` | **No** | Trigger the publish workflow manually instead (see next section) |

After this, finish the release from CI as described below: trigger **Actions → Publish to npm → Run workflow**, or create the GitHub Release yourself.
